...
def main():
  ...
  for i, file in enumerate(test_files):
      ...
      try:
          ...
          # ---------------------- 結果聚合 ----------------------
          # 聚合方式：取平均值
          agg_gender = format_value(np.mean(pred_gender))
          agg_hold = format_value(np.mean(pred_hold))
          
          # 確保多分類概率總和為1
          agg_years = np.mean(years_probs, axis=0)
          agg_years = agg_years / np.sum(agg_years)  # 歸一化
          agg_years = [format_value(x) for x in agg_years]
          
          agg_level = np.mean(level_probs, axis=0)
          agg_level = agg_level / np.sum(agg_level)  # 歸一化
          agg_level = [format_value(x) for x in agg_level]
          ...
          if not idx.empty:
              ...
              for k, col in enumerate(years_cols):
                  if k < len(agg_years):
                      result.loc[idx, col] = agg_years[k]
              
              for k, col in enumerate(level_cols):
                  if k < len(agg_level):
                      result.loc[idx, col] = agg_level[k]
  ...
  for col in result.columns:
      if col != 'unique_id':
          # 將所有數值四捨五入到小數點後四位，並處理異常值
          result[col] = result[col].apply(format_value)
  
  # 顯示處理後的前幾行
  print(f"處理後的前幾行: \n{result.head()}")
  
  # 使用pandas的to_csv方法保存，設置float_format避免科學記號
  result.to_csv(submission_path, index=False, float_format=custom_float_format)
