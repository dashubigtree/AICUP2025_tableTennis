...
def main():
    ...
    for file in datalist:
        ...
        if player_id in train_players:
            ...
            train_modes.extend(mode_repeated)
            ...
            test_modes.extend(mode_repeated)
    
    # 將 mode 加入特徵中
    x_train['mode'] = train_modes
    x_test['mode'] = test_modes
    
    print(f"訓練資料特徵數量: {x_train.shape[1]}")  # 應該是86個特徵 (原始85 + mode)
    ...
    train_data.to_csv('./processed_data/train_86features.csv', index=False)
    
    # 保存測試資料 (包含特徵和標籤)
    test_data = pd.concat([x_test, y_test], axis=1)
    test_data.to_csv('./processed_data/test_86features.csv', index=False)
    
    print("資料已保存至 processed_data 目錄")
    print(f"訓練資料: {train_data.shape[0]} 筆, {train_data.shape[1]} 個欄位")
    print(f"測試資料: {test_data.shape[0]} 筆, {test_data.shape[1]} 個欄位")
    ...
    for file in datalist:
        ...
        target = row[target_mask]
        '''
        這裏看起來沒有用到train_info.csv當中的揮拍mode
        '''
        target_repeated = pd.concat([target] * len(data))
        if player_id in train_players:
            x_train = pd.concat([x_train, data], ignore_index=True)
